      ...
      <div className="space-y-12 p-8 bg-white text-black min-h-screen font-sans selection:bg-black selection:text-white">
            ...
            >
              <div className="flex justify-between items-start mb-6">
                <div>
                   ...
                   <p className="text-[8px] font-bold text-black dark:text-slate-400 uppercase tracking-widest mt-1">ID: REF-{worker.slice(0,3).toUpperCase()}</p>
                </div>
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${status === 'present' ? 'bg-emerald-500 text-white shadow-lg' : 'bg-slate-100 dark:bg-slate-800 text-black dark:text-slate-400'}`}>
                   <UserCheck size={14} />
                </div>
              </div>

              <div className="space-y-4 mb-8">
                 <div className="flex justify-between items-center text-[9px] font-bold uppercase text-black dark:text-slate-400 tracking-widest">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className={`flex-1 py-2.5 rounded-lg text-[9px] font-bold uppercase tracking-widest transition-all ${status === "present" ? "bg-emerald-500 text-white shadow-md" : "bg-slate-50 dark:bg-slate-800 text-black dark:text-slate-400 hover:bg-emerald-50 hover:text-emerald-600"}`}
                >
                  FULL
                </button>
                <button
                  onClick={() => markAttendance(worker, "half-day")}
                  className={`flex-1 py-2.5 rounded-lg text-[9px] font-bold uppercase tracking-widest transition-all ${status === "half-day" ? "bg-amber-400 text-black shadow-md font-extrabold" : "bg-slate-50 dark:bg-slate-800 text-black dark:text-slate-400 hover:bg-amber-50 hover:text-amber-600"}`}
                >
                  HALF
                </button>
                <button
                  onClick={() => markAttendance(worker, "absent")}
                  className={`p-2.5 rounded-lg transition-all ${status === "absent" ? "bg-rose-500 text-white shadow-md rotate-90" : "bg-slate-50 dark:bg-slate-800 text-black dark:text-slate-400 hover:bg-rose-50 hover:text-rose-600"}`}
                >
                  <X size={14} />
                </button>
              </div>
        ...
        <div className="w-[210mm] min-h-[297mm] mx-auto border-[12px] border-slate-50 p-10 md:p-14 rounded-xl shadow-3xl relative overflow-hidden bg-white print:w-full print:min-h-[100vh] print:shadow-none print:border-none">
          ...
          <div className="text-center mb-10 flex flex-col items-center relative z-10">
            ...
            <p className="text-[8px] font-black uppercase tracking-[0.5em] text-black dark:text-slate-400">
          ...
          <table className="w-full text-left relative z-10">
            <thead className="border-b-2 border-slate-100">
              <tr className="text-[8px] font-black uppercase tracking-[0.3em] text-black dark:text-slate-400">
            ...
            <tbody className="divide-y divide-slate-50">
              {weeklySummary.workers.map((w, i) => (
                <tr key={i} className="text-sm font-black group">
                  ...
                  <td className="py-4 text-center text-black dark:text-slate-400">
                    {w.presentDays}{" "}
            ...
            <tfoot>
              <tr className="border-t-4 border-black font-black text-2xl">
                <td
                  colSpan="2"
                  className="py-6 text-right text-black dark:text-slate-400"
                >
                  TOTAL:
                </td>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm flex items-center gap-6 group">
          ...
          <div>
            ...
            <p className="text-[10px] font-bold text-black dark:text-slate-400 uppercase tracking-widest leading-none">মোট কর্মী (Workforce)</p>
          </div>
        </div>
        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm flex items-center gap-6 group">
          ...
          <div>
            ...
            <p className="text-[10px] font-bold text-black dark:text-slate-400 uppercase tracking-widest leading-none">আজ উপস্থিত (Present Today)</p>
          </div>
        </div>
        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm flex items-center gap-6 group">
          ...
          <div>
            ...
            <p className="text-[10px] font-bold text-black dark:text-slate-400 uppercase tracking-widest leading-none">দৈনিক পেমেন্ট (Payments)</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 !p-1.5 flex flex-col lg:flex-row items-center justify-between gap-6 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm my-4">
        <div className="flex flex-wrap gap-1 w-full lg:w-auto overflow-x-auto no-scrollbar">
          {[
            ...
            <button
              ...
              className={`px-6 py-2.5 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all ${selectedDepartment === dept.id ? 'bg-slate-950 text-white shadow-lg' : 'text-black dark:text-slate-400 hover:text-black dark:hover:text-white'}`}
            >
              {dept.label}
            </button>
          ))}
        ...
        <div className="flex items-center gap-3 w-full lg:w-auto">
          <div className="relative group flex-1 lg:flex-none">
            <Calendar size={14} className="absolute left-4 top-1/2 -translate-y-1/2 text-black dark:text-slate-400" />
            <input
              type="date"
              ...
              <div key={idx} className="saas-card flex flex-col md:flex-row justify-between items-center gap-6 group hover:border-slate-950 dark:hover:border-white transition-all animate-fade-up">
                <div className="flex items-center gap-5 flex-1 w-full md:w-auto">
                  ...
                  <div className="space-y-0.5">
                    ...
                    <p className="text-black dark:text-slate-400 text-[10px] font-bold uppercase tracking-widest leading-none mt-1 flex items-center gap-1.5 italic">
