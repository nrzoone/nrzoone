        ...
        <div className="space-y-8 pb-24 animate-fade-up px-1 md:px-2 text-slate-950">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8">
                <div className="flex items-center gap-4">
                    ...
                    <div className="space-y-1">
                        <h2 className="text-3xl font-bold tracking-tight text-slate-950 dark:text-white uppercase leading-none">সিকিউরিটি ও <span className="text-blue-600">অডিট লগ</span></h2>
                        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest italic leading-none mt-1">Audit Trail & System Integrity Hub</p>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                 {[
                    ...
                    { label: 'Audit Density', value: logs.length + ' ইভেন্ট', icon: <Activity size={24} />, color: 'bg-slate-50 text-slate-600 dark:bg-slate-800 dark:text-white' }
                 ].map((stat, i) => (
                    <div key={i} className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-100 dark:border-slate-800 flex items-center gap-6 shadow-sm group">
                        ...
                        <div>
                            <p className="text-2xl font-bold tracking-tight text-slate-950 dark:text-white leading-none mb-1 uppercase">{stat.value}</p>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden">
                <div className="p-6 md:p-8 border-b border-slate-100 dark:border-slate-800 flex flex-col md:flex-row justify-between gap-6 md:items-center bg-slate-50/30 dark:bg-slate-800/20">
                    <div className="flex items-center gap-3">
                        ...
                        <h3 className="text-lg font-bold uppercase tracking-tight text-slate-950 dark:text-white">সিস্টেম অডিট ট্রেইল (Full Audit Trail)</h3>
                    </div>
                                    ...
                                    return (
                                        <tr key={i} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors group">
                                            <td className="px-8 py-6">
                                                <p className="text-sm font-bold text-slate-950 dark:text-white leading-none mb-1">{timeStr}</p>
                                                <p className="text-[9px] font-bold uppercase text-slate-400 tracking-widest leading-none">{dateStr}</p>
                                            </td>
                                            <td className="px-6 py-6">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <p className="text-sm font-bold uppercase text-slate-900 dark:text-white">{log.user || 'SYSTEM'}</p>
                                                </div>
                                            </td>
                                            <td className="px-6 py-6">
                                                <span className="px-3 py-1 bg-slate-100 dark:bg-slate-800 rounded-md text-[9px] font-bold uppercase tracking-widest text-slate-600 dark:text-slate-300 group-hover:bg-slate-950 group-hover:text-white transition-all">
                                                    {log.action || log.type}
                                                </span>
                                            </td>
                                            <td className="px-8 py-6">
                                                <p className="text-xs font-medium text-slate-500 dark:text-slate-400 tracking-tight leading-relaxed lg:max-w-md italic">
                ...
                {logs.length > 100 && (
                    <div className="p-6 text-center bg-slate-50/50 dark:bg-slate-800/30 border-t border-slate-100 dark:border-slate-800">
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest italic">Showing latest 100 audit entries</p>
                    </div>
                )}
            </div>
        </div>
