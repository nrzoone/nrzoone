    ...
    <div className={`mb-4 transition-all duration-300 rounded-xl border border-slate-100 dark:border-slate-800 overflow-hidden ${isOpen ? 'bg-white dark:bg-slate-900 shadow-xl scale-[1.01]' : 'bg-slate-50 dark:bg-slate-800/50 hover:bg-white dark:hover:bg-slate-800'}`}>
      ...
      >
        <div className="flex items-center gap-5">
          <div className={`w-12 h-12 rounded-xl flex items-center justify-center transition-all ${isOpen ? 'bg-slate-950 text-white' : 'bg-white dark:bg-slate-900 text-black dark:text-white group-hover:scale-110 shadow-sm border border-slate-100 dark:border-slate-800'}`}>
            <Icon size={20} />
          </div>
          <div>
            <h3 className={`text-lg font-bold uppercase tracking-tight leading-none transition-colors ${isOpen ? 'text-black dark:text-white' : 'text-black dark:text-white'}`}>{label}</h3>
            <p className="text-[9px] font-bold uppercase text-black dark:text-white tracking-widest mt-2 italic">{description}</p>
          </div>
        </div>
  ...
  if (!isAdmin) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center p-12 text-center animate-fade-up">
        ...
        <p className="text-[10px] font-bold text-black dark:text-white uppercase tracking-widest max-w-xs leading-relaxed italic">
  ...
  const TabButton = ({ id, label, active, onClick }) => (
    <button
      ...
      className={`px-6 py-3 rounded-xl font-bold uppercase text-[10px] tracking-widest transition-all ${
        active === id ? "bg-slate-950 text-white shadow-lg" : "text-black dark:text-white hover:text-black dark:hover:text-white"
      }`}
    >
      {label}
    </button>
      ...
      <div className="bg-white dark:bg-slate-900 p-8 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm flex justify-between items-center">
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <p className="text-[10px] text-black dark:text-white font-bold uppercase tracking-widest mt-2">
              ...
              {isEditing ? (
                <div className="space-y-4 relative z-10 h-full flex flex-col">
                  ...
                  <div className="flex gap-2 mt-auto">
                    <button
                      ...
                      className="flex-1 py-3 rounded-lg bg-slate-50 dark:bg-slate-800 text-[8px] font-bold uppercase tracking-widest text-black dark:text-white"
                    >
                      Cancel
                    </button>
                    <button
                ...
                <>
                  <div className="space-y-1">
                    <p className="text-[8px] font-bold text-black dark:text-white uppercase tracking-widest mb-2">Item ID: {idx + 1}</p>
                  ...
                  <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-all mt-6">
                    <button
                      ...
                      className="w-10 h-10 rounded-xl bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-black dark:text-white hover:text-black dark:hover:text-white transition-all shadow-sm border border-slate-100 dark:border-slate-800"
                    >
                      <Edit2 size={14} />
                    </button>
                    <button
          ...
          {masterData.users?.map((u, idx) => (
             <div key={idx} className="saas-card group relative flex flex-col justify-between overflow-hidden min-h-[220px]">
                ...
                <div className="flex justify-between items-start mb-8 relative z-10">
                   <span className={`px-3 py-1 rounded-lg text-[9px] font-bold uppercase tracking-widest ${u.role === 'admin' ? 'bg-slate-950 text-white shadow-lg' : 'bg-slate-100 dark:bg-slate-800 text-black dark:text-white'}`}>
                      {u.role}
                   </span>
                ...
                <div className="relative z-10">
                   ...
                   <div className="flex items-center gap-3 mt-3">
                      <p className="text-[10px] font-bold text-black dark:text-white uppercase tracking-widest">{u.id}</p>
                ...
                <div className="mt-8 pt-6 border-t border-slate-100 dark:border-slate-800 flex justify-between items-center relative z-10">
                   <div>
                      <p className="text-[8px] font-bold text-black dark:text-white uppercase tracking-tight mb-1">Auth Level</p>
                   ...
                   <button 
                      ...
                      className="px-4 py-2 bg-slate-50 dark:bg-slate-800 text-black dark:text-white hover:text-black dark:hover:text-white rounded-lg text-[8px] font-bold uppercase tracking-widest transition-all"
                   >
                      Verify
                   </button>
                </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {[
          ...
          <button key={sec.label} onClick={() => setActiveTab(sec.cat)} className="flex items-center justify-between p-6 bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800 hover:border-slate-950 transition-all group text-left shadow-sm">
            <div className="flex items-center gap-5">
              <div className="w-12 h-12 bg-slate-50 dark:bg-slate-800 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform"><sec.icon size={18} className="text-black dark:text-white" /></div>
              <div>
                 <h4 className="text-lg font-bold tracking-tight uppercase leading-none text-black dark:text-white">{sec.label}</h4>
                 <p className="text-[9px] font-bold text-black dark:text-white uppercase tracking-widest mt-2">{sec.items?.length || 0} টি কনফিগার করা হয়েছে</p>
              </div>
            </div>
    ...
    <div className="space-y-8 animate-fade-up">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        ...
        <div className="relative w-full md:w-80">
           <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-black dark:text-white" size={18} />
           <input 
              type="text" 
      ...
      {['sewing', 'stone', 'pata'].map(dept => (
        <div key={dept} className="saas-card !bg-slate-50/50 dark:!bg-slate-900/50">
          <div className="flex justify-between items-center mb-6">
             ...
             <span className="text-[10px] font-bold uppercase text-black dark:text-white tracking-widest">{getUnifiedWorkers(dept).length} জন কর্মী</span>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {getUnifiedWorkers(dept)
              ...
              <div key={idx} className="saas-card group relative flex flex-col justify-between overflow-hidden shadow-sm hover:border-slate-950 dark:hover:border-white transition-all min-h-[240px]">
                <div className="flex justify-between items-start mb-6">
                  <div className="w-14 h-14 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-800 flex items-center justify-center text-black dark:text-white group-hover:scale-105 transition-all overflow-hidden shadow-inner shrink-0">
                  ...
                  <div className="flex gap-2">
                    <button 
                      ...
                      className="w-9 h-9 bg-white dark:bg-slate-800 rounded-lg text-black dark:text-white hover:text-black dark:hover:text-white transition-all shadow-sm border border-slate-200 dark:border-slate-700 flex items-center justify-center"
                    >
                      <Edit2 size={14} />
                    </button>
                ...
                <div className="space-y-1 mb-6">
                   ...
                   <p className="text-[9px] font-bold text-black dark:text-white uppercase tracking-widest italic">{w.phone || "No Connection"}</p>
                </div>

                <div className="mt-auto pt-6 border-t border-slate-100 dark:border-slate-800 flex justify-between items-end">
                   <div>
                      <p className="text-[8px] font-bold text-black dark:text-white uppercase tracking-tight mb-1">বেস রেট (Wage)</p>
                      <p className="text-xl font-bold text-black dark:text-white">৳{w.wage}</p>
                   </div>
                   <div className="flex gap-2">
                      <button 
                        onClick={() => setPrintWorkerDoc(w)} 
                        className="w-10 h-10 rounded-xl bg-slate-50 dark:bg-slate-800 text-black dark:text-white hover:bg-slate-950 hover:text-white transition-all shadow-sm border border-slate-100 dark:border-slate-800 flex items-center justify-center"
                      >
                         <Printer size={16} />
                      </button>
    ...
    <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800 overflow-hidden min-h-[400px] shadow-sm animate-fade-up">
      <div className="overflow-x-auto">
        <table className="w-full text-left">
          <thead className="bg-slate-50 dark:bg-slate-800/50 text-black dark:text-white font-bold text-[10px] uppercase tracking-widest border-b border-slate-100 dark:border-slate-800">
            <tr>
            ...
            {(masterData.auditLogs || []).slice(0, 50).map((log, i) => (
              <tr key={i} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-all">
                <td className="px-6 py-4">
                   ...
                   <p className="text-[9px] font-bold text-black dark:text-white">{new Date(log.timestamp).toLocaleDateString()}</p>
                </td>
                <td className="px-6 py-4">
                   <p className="text-xs font-bold uppercase text-black dark:text-white">{log.user}</p>
                   <span className="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-800 text-[8px] font-bold text-black dark:text-white uppercase tracking-widest rounded">{log.role}</span>
                </td>
                <td className="px-6 py-4">
                    <span className="text-[10px] font-bold uppercase text-blue-600 dark:text-blue-400">{log.action}</span>
                </td>
                <td className="px-6 py-4 text-[10px] text-black dark:text-white truncate max-w-xs">{typeof log.details === 'string' ? log.details : JSON.stringify(log.details)}</td>
              </tr>
            ))}
          </tbody>
        </table>
    ...
    <div className="space-y-6 animate-fade-up">
       ...
       <div className="saas-card flex flex-col md:flex-row items-center gap-10 group hover:border-slate-950 dark:hover:border-white transition-all duration-500">
          <div className="w-40 h-40 bg-slate-50 dark:bg-slate-800 rounded-xl shadow-inner flex items-center justify-center overflow-hidden border border-slate-100 dark:border-slate-700 relative shrink-0">
             {masterData.settings?.logo ? (
                ...
                <div className="flex flex-col items-center gap-4 text-slate-300">
                   <ImageIcon size={40} />
                   <p className="text-[9px] font-bold uppercase tracking-widest text-black dark:text-white">No Custom Logo</p>
                </div>
             )}
             {uploading && (
          ...
          <div className="flex-1 text-center md:text-left">
             ...
             <p className="text-black dark:text-white text-xs font-bold leading-relaxed mb-8 italic">
  ...
  const renderDatabaseContent = () => (
    <div className="space-y-6 animate-fade-up">
      <div className="bg-slate-950 text-white rounded-xl p-10 text-center relative overflow-hidden group shadow-2xl">
         <div className="relative z-10">
            <h4 className="text-3xl font-bold uppercase tracking-tight mb-3">মাস্টার <span className="text-black dark:text-white">আর্কাইভ (Archives)</span></h4>
            <p className="text-black dark:text-white text-[10px] font-bold uppercase tracking-widest max-w-md mx-auto mb-10 italic">তথ্য সুরক্ষিত রাখুন এবং প্রয়োজনে রিস্টোর করুন।</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
         {['productions', 'cuttingStock', 'pataEntries', 'deliveries', 'attendance'].map(key => (
            <div key={key} className="saas-card flex justify-between items-center group hover:border-rose-500 transition-all">
               <div>
                  <p className="text-[9px] font-bold text-black dark:text-white uppercase tracking-widest mb-1">{key.toUpperCase()}</p>
    ...
    <div className="space-y-6 pb-24 animate-fade-up px-2">
      ...
      <div className="bg-white dark:bg-slate-900 px-8 py-6 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col md:flex-row justify-between items-center gap-6 mb-8 mt-2">
        <div className="flex items-center gap-6">
          <button
            onClick={() => setActivePanel("Overview")}
            className="w-12 h-12 flex items-center justify-center bg-slate-50 dark:bg-slate-800 text-black dark:text-white hover:bg-slate-950 hover:text-white rounded-xl transition-all shadow-sm border border-slate-100 dark:border-slate-800"
          >
            <ArrowLeft size={20} />
          </button>
          <div className="text-left">
            ...
            <p className="text-[10px] font-bold text-black dark:text-white uppercase tracking-widest mt-2">
          ...
          {[
            ...
            <button
              ...
              className={`w-full flex items-center justify-between p-4 rounded-xl transition-all group ${
                ...
                : 'hover:bg-white dark:hover:bg-slate-800 text-black dark:text-white hover:text-black dark:hover:text-white border border-transparent border-slate-100 dark:border-slate-800'
              }`}
            >
      ...
      {showAddModal === "user" && (
        <div className="fixed inset-0 bg-slate-950/40 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
          <div className="saas-card !p-12 w-full max-w-lg shadow-2xl animate-fade-up">
            ...
            <p className="text-[10px] font-bold text-black dark:text-white uppercase tracking-widest mb-10">Access Provisioning Control</p>
            <div className="space-y-6">
              ...
              <div className="relative">
                ...
                <button onClick={() => setShowUserPass(!showUserPass)} className="absolute right-4 top-1/2 -translate-y-1/2 text-black dark:text-white">
              ...
              <div className="flex gap-4 pt-6">
                <button onClick={() => setShowAddModal(false)} className="flex-1 py-4 rounded-xl font-bold uppercase text-[10px] tracking-widest text-black dark:text-white hover:bg-slate-50 transition-all">Cancel</button>
                <button 
      ...
      {showAddModal === "worker" && (
        <div className="fixed inset-0 bg-slate-950/40 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
          <div className="saas-card !p-12 w-full max-w-lg shadow-2xl animate-fade-up">
            ...
            <p className="text-[10px] font-bold text-black dark:text-white uppercase tracking-widest mb-10">Workforce Deployment Node</p>
            <div className="space-y-6">
              <select
                id="new-worker-dept"
      ...
      {(showAddModal === "design" || editDesignModal) && (
        <div className="fixed inset-0 bg-slate-950/40 backdrop-blur-sm z-[200] flex items-center justify-center p-4 overflow-y-auto">
          <div className="saas-card !p-10 w-full max-w-2xl shadow-2xl animate-fade-up my-auto">
            ...
            <p className="text-[10px] font-bold text-black dark:text-white uppercase tracking-widest mb-8">Product Development Architecture</p>
            ...
            <div className="flex flex-col md:flex-row gap-8">
                 ...
                 <label className="w-full aspect-square bg-slate-50 dark:bg-slate-800 rounded-xl border border-dashed border-slate-200 dark:border-slate-700 flex flex-col items-center justify-center cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800 transition-all overflow-hidden group">
                    {tempImgUrl ? (
                      ...
                      <div className="text-center p-4">
                        <Upload size={24} className="mx-auto text-black dark:text-white mb-2 group-hover:scale-110 transition-transform" />
                        <span className="text-[8px] font-bold uppercase text-black dark:text-white tracking-widest">Upload Photo</span>
                      </div>
                    )}
              ...
              <div className="flex-1 space-y-4">
                 ...
                 <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="text-[8px] font-bold text-black dark:text-white uppercase ml-1">Sewing Rate</label>
                      <input id="design-sewing" type="number" className="premium-input !h-11" defaultValue={editDesignModal?.sewingRate || 0} />
                    </div>
                    <div>
                      <label className="text-[8px] font-bold text-black dark:text-white uppercase ml-1">Stone Rate</label>
                      <input id="design-stone" type="number" className="premium-input !h-11" defaultValue={editDesignModal?.stoneRate || 0} />
                    </div>
                    <div>
                      <label className="text-[8px] font-bold text-black dark:text-white uppercase ml-1">Pata Rate</label>
                      <input id="design-pata" type="number" className="premium-input !h-11" defaultValue={editDesignModal?.pataRate || 0} />
                    </div>
                    <div>
                      <label className="text-[8px] font-bold text-black dark:text-white uppercase ml-1">Sell Price</label>
            ...
            <div className="flex gap-4 pt-8">
              <button onClick={() => { setShowAddModal(false); setEditDesignModal(null); setTempImgUrl(null); }} className="flex-1 py-4 rounded-xl font-bold uppercase text-[10px] tracking-widest text-black dark:text-white hover:bg-slate-50 transition-all">Cancel</button>
              <button 
                onClick={() => {
                  const data = {
