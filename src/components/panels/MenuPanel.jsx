...
const MenuPanel = ({ setActivePanel, user, t, showNotify, lowStockItems }) => {
    const categories = [
        ...
        },
        {
            ...
            items: [
                { id: 'Cutting', label: 'কাটিং', icon: <Scissors size={32} />, color: 'bg-[var(--text-primary)]', desc: 'র-কাট লট ম্যানেজমেন্ট' },
                { id: 'Swing', label: 'সেলাই', icon: <Layers size={32} />, color: 'bg-[var(--text-primary)]', desc: 'ফ্যাক্টরি ফ্লোর অপারেশন' },
                { id: 'Stone', label: 'স্টোন', icon: <Hammer size={32} />, color: 'bg-[var(--text-primary)]', desc: 'স্টোন ইউনিট কন্ট্রোল' },
                { id: 'Pata', label: 'পাতা', icon: <Package size={32} />, color: 'bg-[var(--text-primary)]', desc: 'লজিস্টিকস এবং স্টোরেজ' }
            ]
        },
        {
        ...
        },
        {
            ...
            items: [
                { id: 'Settings', label: 'সেটিংস', icon: <Settings size={32} />, color: 'bg-[var(--text-primary)]', desc: 'মাস্টার সেটআপ কন্ট্রোল' }
            ]
        }
    ];

    return (
        <div className="fixed inset-0 z-[9999] bg-slate-50 dark:bg-slate-950 p-6 md:p-24 overflow-y-auto animate-fade-in font-outfit text-[var(--text-primary)]">
            <div className="max-w-7xl mx-auto pb-48">
                <div className="flex justify-between items-end mb-24 md:mb-32 animate-elite">
                    <div className="flex items-center gap-10">
                        <div className="w-20 h-20 md:w-32 md:h-32 bg-slate-950 text-white dark:bg-white dark:text-[var(--bg-primary)] rounded-[2.5rem] md:rounded-[4rem] flex items-center justify-center shadow-2xl border-2 border-slate-200 dark:border-slate-800">
                            <LayoutGrid size={48} className="md:w-16 md:h-16" />
                        </div>
                        <div className="space-y-4">
                            <h1 className="text-5xl md:text-8xl font-black uppercase tracking-normal leading-none text-[var(--text-primary)]">মেনু হাব</h1>
                            <p className="text-[10px] md:text-[12px] font-bold text-[var(--text-muted)] uppercase tracking-[0.3em]">NRZOONE WOMEN'S CLOTHING — ELITE ACCESS</p>
                        </div>
                    </div>
                    <button 
                        onClick={() => setActivePanel('Overview')}
                        className="w-20 h-20 md:w-28 md:h-28 rounded-full bg-slate-950 text-white dark:bg-white dark:text-[var(--bg-primary)] shadow-2xl flex items-center justify-center hover:scale-110 active:scale-95 transition-all group"
                    >
                        ...
                        return (
                            <div key={i} className="animate-elite" style={{ animationDelay: `${i * 150}ms` }}>
                                <div className="flex items-center gap-6 mb-10">
                                    <h3 className="text-[11px] md:text-[13px] font-black uppercase tracking-[0.4em] text-[var(--text-primary)] whitespace-nowrap">{cat.title}</h3>
                ...
                <div className="mt-48 pt-20 border-t border-slate-200 dark:border-slate-800 flex flex-col md:flex-row justify-between items-center opacity-60 gap-10 text-[var(--text-muted)]">
                    <p className="text-[11px] font-bold uppercase tracking-[0.6em]">NRZOONE COUTURE ENGINE V5.2 // SECURE BOOTH</p>
                    <div className="flex gap-16">
                        <button onClick={() => setActivePanel('Security')} className="text-[11px] font-bold uppercase tracking-[0.6em] hover:text-[var(--text-primary)]">SYSTEM LOGS</button>
                        <button onClick={() => showNotify('NRZONE NEURAL ENGINE: STATUS ONLINE')} className="text-[11px] font-bold uppercase tracking-[0.6em] hover:text-[var(--text-primary)]">GATEWAY STATUS</button>
                    </div>
                </div>
            </div>
        </div>
