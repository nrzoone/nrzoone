          ...
          <div className="border-[6px] border-black p-12 md:p-20 rounded-3xl relative overflow-hidden bg-white">
            <div className="flex justify-between items-start border-b-[6px] border-black pb-10 mb-10">
              <div>
                ...
                <p className="text-[10px] font-black uppercase tracking-[0.5em] text-black dark:text-slate-400">
              ...
              {filteredExpenses.map((exp, i) => (
                ...
                >
                  <div className="space-y-1">
                    ...
                    <p className="text-[9px] font-black text-black dark:text-slate-400 tracking-widest opacity-60">
                      {exp.category}
                    </p>
                  </div>
            ...
            <div className="mt-16 pt-10 border-t-[6px] border-black flex justify-between items-center">
              <p className="text-2xl font-black text-black dark:text-slate-400 uppercase">
                Daily Outflow Total
              </p>
    ...
    <div className="space-y-10 pb-32 animate-fade-up px-1 md:px-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
        <div className="saas-card !p-8 flex items-center gap-8 group transition-all hover:border-emerald-500/30">
          ...
          <div>
            <p className="text-3xl font-black tracking-tight text-black dark:text-white leading-none mb-1">৳{currentBalance.toLocaleString()}</p>
            <p className="text-[10px] font-bold text-black dark:text-slate-400 uppercase tracking-widest leading-none italic">Available Cash</p>
          </div>
        </div>
        <div className="saas-card !p-8 flex items-center gap-8 group transition-all hover:border-rose-500/30">
          ...
          <div>
            <p className="text-3xl font-black tracking-tight text-black dark:text-white leading-none mb-1">৳{totalExpenses.toLocaleString()}</p>
            <p className="text-[10px] font-bold text-black dark:text-slate-400 uppercase tracking-widest leading-none italic">Burn Rate (Total)</p>
          </div>
        </div>
        <button 
      ...
      <div className="saas-card !p-4 flex flex-col lg:flex-row items-center justify-between gap-6 mb-10 border-blue-500/10">
        <div className="pill-nav w-full lg:w-auto overflow-x-auto no-scrollbar">
          {[
            ...
            <button
              ...
              className={`pill-tab ${activeTab === v.id ? 'pill-tab-active' : 'text-black dark:text-slate-400 hover:text-black dark:hover:text-white'}`}
            >
              {v.label}
            </button>
          ))}
        ...
        <div className="flex items-center gap-4 w-full lg:w-auto">
          <div className="relative group flex-1 lg:flex-none">
            <Search size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-black dark:text-slate-400" />
            <input
        ...
        <div className="flex justify-center animate-fade-up">
           <div className="saas-card w-full max-w-2xl !p-12 space-y-10">
              <div className="text-center space-y-3">
                  ...
                  <h3 className="text-3xl font-black tracking-tight text-black dark:text-white leading-none italic uppercase italic">টাকা খরচ (Expense)</h3>
                  <p className="text-[10px] font-bold text-black dark:text-slate-400 uppercase tracking-widest italic leading-none">Record money going out of the business</p>
              </div>
              <form onSubmit={handleAddExpense} className="space-y-8">
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                        <label className="text-[10px] font-bold text-black dark:text-slate-400 ml-4 uppercase italic">Category</label>
                    ...
                    <div className="space-y-2">
                        <label className="text-[10px] font-bold text-black dark:text-slate-400 ml-4 uppercase italic">Date</label>
                 ...
                 <div className="space-y-2">
                    <label className="text-[10px] font-bold text-black dark:text-slate-400 ml-4 uppercase italic">Description</label>
        ...
        <div className="flex justify-center animate-fade-up">
           <div className="saas-card w-full max-w-2xl !p-12 space-y-10">
              <div className="text-center space-y-3">
                  ...
                  <h3 className="text-3xl font-black tracking-tight text-black dark:text-white leading-none italic uppercase italic">টাকা আসা (Cash In)</h3>
                  <p className="text-[10px] font-bold text-black dark:text-slate-400 uppercase tracking-widest italic leading-none">Record money coming into the business</p>
              </div>
              <form onSubmit={handleAddCash} className="space-y-8">
                 ...
                 <div className="space-y-2">
                    <label className="text-[10px] font-bold text-black dark:text-slate-400 ml-4 uppercase italic">Source / Details</label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredExpenses.length === 0 ? (
              <div className="col-span-full h-80 flex flex-col items-center justify-center saas-card border-2 border-dashed border-slate-200">
                ...
                <p className="text-[11px] font-bold text-black dark:text-slate-400 uppercase tracking-widest italic">No Consumption Records Audited For This Date</p>
              </div>
            ) : (
              filteredExpenses.map((exp, idx) => (
                <div key={exp.id || idx} className="saas-card !p-8 flex flex-col justify-between h-80 group hover:border-slate-900 transition-all relative overflow-hidden">
                  <div className="absolute top-0 right-0 p-10 opacity-[0.03] group-hover:opacity-[0.08] transition-opacity">
                    <DollarSign size={160} className="text-black dark:text-white" />
                  </div>
                  <div className="relative z-10">
                    <div className="flex justify-between items-start mb-6">
                      <span className="px-3 py-1 bg-slate-100 dark:bg-slate-800 rounded-lg text-[9px] font-bold text-black dark:text-slate-400 uppercase tracking-widest border border-slate-200 dark:border-slate-700">
                        {exp.category}
                      </span>
                      <button onClick={() => setEditExpense(exp)} className="w-10 h-10 bg-slate-50 dark:bg-slate-800 rounded-xl flex items-center justify-center text-black dark:text-slate-400 hover:bg-slate-950 hover:text-white transition-all shadow-sm">
                        <Edit2 size={14} />
                      </button>
                    </div>
                    <h5 className="text-2xl font-black tracking-tight text-black dark:text-white uppercase leading-tight italic mb-2">
                      {exp.description}
                    </h5>
                    <p className="text-[10px] font-bold text-black dark:text-slate-400 uppercase tracking-widest flex items-center gap-2 italic">
                      <Calendar size={12} /> {exp.date}
                    </p>
                  </div>
                  <div className="relative z-10 flex justify-between items-end">
                    <p className="text-4xl font-black text-black dark:text-white italic">
                      ৳{exp.amount.toLocaleString()}
                    </p>
      ...
      <div className="flex justify-center pt-24 pb-12">
        ...
        >
          ...
          <span className="text-xl font-black uppercase italic tracking-[0.2em] text-black dark:text-white">
            Return to Core
          </span>
        </button>
      </div>
        ...
        <div className="fixed inset-0 bg-slate-950/40 backdrop-blur-3xl z-[300] flex items-center justify-center p-4">
           <div className="bg-white w-full max-w-lg rounded-3xl border-4 border-slate-50 shadow-3xl p-8 md:p-12 space-y-12 animate-fade-up text-black italic italic">
                <div className="text-center space-y-3">
                  ...
                  <p className="text-[10px] font-bold text-black dark:text-slate-400 uppercase tracking-[0.4em]">Audit Trail Correction</p>
                </div>
                <form onSubmit={handleUpdateExpense} className="space-y-8">
                   <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <label className="text-[10px] font-bold text-black dark:text-slate-400 ml-4 uppercase tracking-widest italic">Date</label>
                      ...
                      <div className="space-y-2">
                        <label className="text-[10px] font-bold text-black dark:text-slate-400 ml-4 uppercase tracking-widest italic">Category</label>
                   ...
                   <div className="space-y-2">
                      <label className="text-[10px] font-bold text-black dark:text-slate-400 ml-4 uppercase tracking-widest italic">Description</label>
                   ...
                   <div className="flex gap-4">
                      <button type="button" onClick={() => setEditExpense(null)} className="flex-1 py-6 rounded-xl font-black text-[10px] uppercase tracking-[0.2em] bg-white border-2 border-black text-black hover:bg-black hover:text-white transition-all shadow-lg">বাতিল (Cancel)</button>
