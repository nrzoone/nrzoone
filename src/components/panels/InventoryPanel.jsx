...
const InventoryPanel = ({
  ...
  return (
    <div className="space-y-8 pb-32 animate-fade-up px-1 md:px-4 text-slate-900 border-none">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8">
        <div className="flex items-center gap-5">
          ...
          <div>
            ...
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-2 italic leading-none">
      ...
      <div className="bg-white dark:bg-slate-900 !p-1.5 flex flex-wrap gap-1 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm overflow-x-auto no-scrollbar">
        {[
          ...
          <button
            ...
            className={`flex-1 min-w-[120px] px-6 py-3 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all ${view === v.id ? "bg-slate-950 text-white shadow-lg" : "text-slate-400 hover:text-slate-900 dark:hover:text-white"}`}
          >
            {v.label}
          </button>
        ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {summary.length === 0 ? (
                    <div className="col-span-full py-40 flex flex-col items-center justify-center saas-card border-dashed">
                        ...
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">বর্তমানে কোনো তৈরি মাল স্টকে নেই</p>
                    </div>
                ) : (
                        ...
                        <div key={idx} className="saas-card group hover:border-slate-950 dark:hover:border-white transition-all animate-fade-up">
                            <div className="flex justify-between items-start mb-8">
                                <div className="space-y-1">
                                    <h4 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white uppercase leading-none truncate max-w-[200px]">{item.design}</h4>
                                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest italic">• {item.color} ({item.size})</p>
                                </div>
                            ...
                            <div className="grid grid-cols-2 gap-4 border-t border-slate-100 dark:border-slate-800 pt-6">
                                <div className="space-y-1">
                                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest leading-none">বোরকা (Borka)</p>
                                    <p className="text-3xl font-bold text-slate-950 dark:text-white leading-none">{item.borka} পিস</p>
                                </div>
                                <div className="space-y-1 border-l border-slate-100 dark:border-slate-800 pl-4">
                                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest leading-none">হিজাব (Hijab)</p>
                                    <p className="text-3xl font-bold text-slate-400 leading-none">{item.hijab} পিস</p>
                                </div>
                            </div>
                        </div>
                    ))
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {filteredInventory.length === 0 ? (
                        <div className="col-span-full py-32 flex flex-col items-center justify-center saas-card border-dashed">
                            ...
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">কোনো কাঁচামাল পাওয়া যায়নি</p>
                        </div>
                    ) : (
                        filteredInventory.map((item, idx) => (
                            <div key={idx} className="saas-card border-transparent hover:border-slate-950 dark:hover:border-white transition-all group">
                                <h4 className="text-lg font-bold tracking-tight text-slate-950 dark:text-white uppercase truncate mb-1">
                                    {item.name}
                                </h4>
                                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-6 italic truncate">
                                    {item.color || "STANDARD GRADE"}
                                </p>
                                <div className="flex items-end justify-between">
                                    <span className={`text-4xl font-bold tracking-tighter leading-none ${item.qty <= 5 ? "text-rose-600" : "text-slate-950 dark:text-white"}`}>
                                        {item.qty.toLocaleString()}
                                    </span>
                                    <span className={`text-[8px] font-black py-1 px-2.5 rounded-lg tracking-[0.15em] uppercase ${item.qty <= 5 ? "bg-rose-500 text-white animate-pulse" : "bg-slate-100 dark:bg-slate-800 text-slate-500"}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {(masterData.whatsappRequests || []).length === 0 ? (
                    <div className="col-span-full py-40 flex flex-col items-center justify-center saas-card border-dashed">
                        ...
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.4em]">বর্তমানে কোনো রিকোয়েস্ট নেই</p>
                    </div>
                ) : (
                    masterData.whatsappRequests.map((req, idx) => (
                        <div key={idx} className="saas-card border-l-4 border-l-blue-600 group hover:border-blue-700 transition-all animate-fade-up">
                            <div className="flex justify-between items-start mb-8">
                                <div className="space-y-1">
                                    <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest leading-none mb-1">প্রেরক (Operator)</p>
                                    <h4 className="text-xl font-bold uppercase tracking-tight text-slate-950 dark:text-white">{req.worker}</h4>
                                </div>
                            ...
                            <div className="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-xl border border-slate-100 dark:border-slate-800 mb-8">
                                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-2 italic">প্রয়োজনীয় মালামাল</p>
                                <p className="text-2xl font-bold tracking-tight text-slate-950 dark:text-white uppercase leading-none">{req.item} <span className="text-sm text-slate-400 font-bold ml-2">x {req.qty}</span></p>
                            </div>
      ...
      <div className="flex justify-center pt-24 pb-12">
        ...
        >
            ...
            <span className="text-sm lg:text-lg font-bold tracking-tight text-slate-950 dark:text-white uppercase leading-none">
          ...
          <div className="fixed inset-0 bg-slate-950/40 backdrop-blur-md z-[1000] flex items-center justify-center p-4">
            <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-xl shadow-2xl p-8 md:p-12 space-y-10 border border-slate-100 dark:border-slate-800 animate-fade-up">
              <div className="text-center space-y-3">
                ...
                <div>
                    <h3 className="text-2xl font-bold text-slate-950 dark:text-white uppercase leading-none">
                        স্টক {transactionType === "in" ? "যোগ (Entry)" : "কমানো (Out)"}
                    </h3>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-2 italic leading-none">Inventory Adjustments Protocol</p>
                </div>
              </div>
              ...
              <form onSubmit={handleTransaction} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-1.5">
                        <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest ml-1">আইটেম (Material)</label>
                        <input name="item" list="items-list" className="premium-input !h-12 text-sm uppercase font-bold" placeholder="MATERIAL..." required />
                    </div>
                    <div className="space-y-1.5">
                        <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest ml-1">তারিখ (Date)</label>
                ...
                <div className="space-y-1.5">
                    <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest ml-1">গ্রেড / কালার (Classification)</label>
                ...
                <div className="flex flex-col md:flex-row gap-4 pt-4">
                    <button type="button" onClick={() => setShowModal(false)} className="flex-1 py-4 rounded-xl font-bold text-[10px] uppercase tracking-widest bg-slate-50 dark:bg-slate-800 text-slate-400 hover:text-rose-500 transition-all border border-slate-100 dark:border-slate-800">বাতিল করুন</button>
        ...
        {showAIScan && (
            <div className="fixed inset-0 z-[2000] bg-slate-950/90 backdrop-blur-3xl flex items-center justify-center p-4">
                <div className="w-full max-w-4xl bg-white dark:bg-slate-950 rounded-xl overflow-hidden shadow-[0_0_100px_rgba(37,99,235,0.2)] border border-white/10 relative animate-fade-up">
                    <button onClick={() => setShowAIScan(false)} className="absolute top-8 right-8 p-4 bg-slate-50 dark:bg-slate-900 rounded-full hover:bg-rose-500 hover:text-white transition-all text-slate-400 z-50">
                        <X size={24} />
                    </button>
                            ...
                            <div className="absolute inset-0 flex items-center justify-center">
                                {identifying ? (
                                    <div className="space-y-8 flex flex-col items-center">
                                        ...
                                        <p className="text-[10px] font-black uppercase text-slate-400 tracking-[0.6em] animate-pulse">Scanning Visual Matrices...</p>
                                    </div>
                                ) : (
                                    <div className="animate-fade-up text-center space-y-8 p-12">
                                        ...
                                        <h4 className="text-6xl md:text-8xl font-black italic tracking-tighter uppercase text-slate-950 dark:text-white">{masterData.designs?.[0]?.name || 'Unknown Node'}</h4>
                                        <div className="flex justify-center gap-12 text-slate-950 dark:text-white">
