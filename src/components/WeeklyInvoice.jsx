...
const WeeklyInvoice = ({ masterData, user, logAction, showNotify, setActivePanel, SafeText }) => {
                            ...
                            {workers.map(w => {
                                ...
                                return (
                                    <tr key={w} className="border-b-2 border-gray-100 font-black italic">
                                        <td className="py-6 uppercase text-xl"><SafeText data={w} /></td>
                                        <td className="py-6 text-center text-2xl"><SafeText data={d.qty} /> <span className="text-xs uppercase text-gray-200">{d.qtyLabel === 'Pcs' ? 'পিস' : 'দিন'}</span></td>
                                        <td className="py-6 text-right text-3xl">৳{typeof d.bill === 'number' ? d.bill.toLocaleString() : <SafeText data={d.bill} />}</td>
                                    </tr>
                                );
                            })}
                        </tbody>
                        ...
                        {workers.map(w => {
                            ...
                            return (
                                <tr key={w} className="hover:bg-slate-50 transition-all group">
                                    <td className="px-16 py-12"><p className="font-black text-3xl uppercase tracking-tighter italic text-black"><SafeText data={w} /></p></td>
                                    <td className="px-16 py-12 text-center font-black text-4xl italic text-black"><SafeText data={d.qty} /> <span className="text-xs text-black dark:text-white dark:text-white">{d.qtyLabel === 'Pcs' ? 'পিস' : 'দিন'}</span></td>
                                    <td className="px-16 py-12 text-right font-black text-5xl italic tracking-tighter text-black">৳{typeof d.bill === 'number' ? d.bill.toLocaleString() : <SafeText data={d.bill} />}</td>
                                </tr>
                            );
                        })}
                    </tbody>
